...
Usage:
    python -m src [--functions_definition PATH] [--input PATH] [--output PATH] [--model ID]
"""
...
def parse_arguments() -> argparse.Namespace:
    """Parse command-line arguments to configure paths and model."""
...
def run_engine() -> int:
    ...
    def display_progress(index: int, call: FunctionCall) -> None:
        print(f"[Prompt {index + 1}/{len(prompts_list)}] Generated: {call.name}({call.parameters})", file=sys.stderr)

    # 3. Start Constrained Decoding
    try:
        decoded_results = decode_all(prompts_list, functions_list, ai_model, on_result=display_progress)
    ...
    print(f"[*] Successfully saved {len(decoded_results)} function calls to {args.output}", file=sys.stderr)
    return 0
