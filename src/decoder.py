"""Constrained decoding engine: Forces the
LLM to output valid JSON matching a schema."""
...
def is_number_valid(
    ...
    """Checks if the generated text is a
    structurally valid number or prefix."""
    complete_pattern = (
        PATTERN_FLOAT_COMPLETE if allow_decimals
        else PATTERN_INT_COMPLETE
        )
    partial_pattern = (
        PATTERN_FLOAT_PARTIAL if allow_decimals
        else PATTERN_INT_PARTIAL
        )

    if termination_char in text_chunk:
        # If we hit the end character (e.g., ',' or '}'),
        # the number before it must be complete
...
def format_available_functions(
    ...
    """Formats the function schemas into a
    readable text block for the prompt."""
    formatted_lines = []
    for func in functions_list:
        args_str = ", ".join(
            f"{name}: {dtype}" for name, dtype in
            func.ordered_parameters()
            )
...
class ConstrainedDecoder:
    ...
    def generate(
        ...
        selected_func_name = self._decode_choice(
            forced_start, list(functions_map.keys())
            )
        target_function = functions_map[selected_func_name]

        # 2. Force the parameters structure
        generation_state = current_context + (
            f'{{"name": "{selected_func_name}", "parameters": {{'
            )
    ...
    def _decode_value(
        ...
        number_str = self._decode_number(
            context, param_type == "number",
            termination_char
            )

        parsed_number = (
            float(number_str) if param_type == "number"
            else int(number_str)
        )
    ...
    def _decode_choice(
        ...
        """Forces the LLM to output one of
        the explicitly provided string options."""
        generated_text = ""
        while generated_text not in valid_options:
            def is_valid_prefix(new_token: str) -> bool:
                ...
                return (
                    any(
                        option.startswith(combined_text)
                        for option in valid_options
                        )
                    )

            generated_text += self._get_most_likely_valid_token(
                context + generated_text, is_valid_prefix
                )

            if len(generated_text) > MAX_TOKEN_LENGTH:
                raise DecodeError(
                    f"Runaway choice generation: {generated_text!r}"
                    )
    ...
    def _decode_string(
        ...
        while find_string_closure(generated_piece) < 0:
            ...
            generated_piece += self._get_most_likely_valid_token(
                context + generated_piece, is_structurally_sound
                )

            if len(generated_piece) > MAX_TOKEN_LENGTH:
                raise DecodeError(
                    f"Runaway string generation: {generated_piece!r}"
                    )
    ...
    def _decode_number(
        ...
        while termination_char not in generated_piece:
            ...
            if len(generated_piece) > MAX_TOKEN_LENGTH:
                raise DecodeError(
                    f"Runaway number generation: {generated_piece!r}"
                    )

        return (
            generated_piece[: generated_piece.index(termination_char)]
            )

    def _get_most_likely_valid_token(
        self,
        current_context: str,
