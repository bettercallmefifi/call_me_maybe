...
are mapped to printable unicode characters in the vocabulary. To avoid using
the slow, native decode() method during logit masking, we reverse this mapping
and cache the utf-8 decoded strings.
"""
...
class Vocabulary:
    ...
    back to their actual string representations."""
    ...
    def __init__(self, vocab_path: str) -> None:
        ...
        # Step 2: Prepare the reverse mapping
        # (Unicode char -> Original Raw Byte)
        forward_mapping = build_byte_to_unicode_mapping()
        unicode_to_byte = {
            unicode_char: byte for byte, unicode_char
            in forward_mapping.items()
            }

        # Step 3: Build the fast lookup cache
        # (Token ID -> Decoded UTF-8 String)
        ...
        for token_str, token_id in token_to_id.items():
            try:
                ...
                ''' Skip tokens that do not form valid
                standalone UTF-8 sequences.'''
                '''We strictly enforce ASCII/valid
                UTF-8 for JSON generation anyway.'''
                continue

    @classmethod
    def from_model(cls, ai_model: Any) -> Vocabulary:
        """Instantiates the vocabulary cache
        directly from the loaded AI model."""
        return cls(ai_model.get_path_to_vocab_file())

    def decode(self, token_ids: list[int]) -> str:
        """Reconstructs text from a sequence
        of token IDs using the local cache."""
        return "".join(
